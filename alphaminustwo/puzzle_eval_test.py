...
def test_evaluate_puzzle_structure():
    ...
    result = evaluate_puzzle(model, puzzle, temperature=0.0)

    # Check result structure
    assert isinstance(result, PuzzleResult)
    assert result.puzzle_id == "test_puzzle"
    assert result.puzzle_rating == 1743
    assert isinstance(result.solved, bool)
    assert isinstance(result.predicted_moves, str)
    assert isinstance(result.expected_moves, list)
    assert len(result.expected_moves) > 0
    assert result.fen is not None


def test_puzzle_expected_moves():
    """Test that puzzle correctly extracts expected moves."""
    model_cfg = GPT124M()
    model = GPT(model_cfg)
    model.eval()

    # Puzzle where first move is setup, second is solution
    puzzle = {
        "id": "test",
        "fen": "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",
        "moves": ["e2e4", "e7e5", "g1f3"],  # Setup, then solution moves
        "rating": 1500,
        "themes": "opening",
    }

    result = evaluate_puzzle(model, puzzle)

    # Expected moves should be everything after the setup move
    assert result.expected_moves == ["e7e5", "g1f3"]
